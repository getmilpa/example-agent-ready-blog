 ...
 * exposing the richer workflow entities (states, raw transitions, gates) that a caller building
 * an actual `publish_post`-style process — or a future gate/artifact layer — needs.
 *
 ...
 * for why a full-graph acyclic check would incorrectly reject `review_gate --reject--> draft
 * --submit--> review_gate`, which is the intended revise-and-resubmit loop).
 */
final class ProcessDefinition implements DefinitionContract
{
     ...
     * checkpoint per state, however many outcomes it offers) — this is how
     * `Milpa\ExampleBlog\Orchestrator\HumanGate` (a later slice) finds the single gate to open
     * for a process instance sitting at `$state`.
     */
     ...
     * attached gate ("ungated" transitions) — a gated state such as `review_gate` is a decision
     * point, and a human resolving it is what breaks the loop each time, so `draft --submit-->
     * review_gate --reject--> draft` is a legitimate, intentional revise-and-resubmit cycle, not
     * a defect. A cycle entirely among ungated (automatic) transitions, however, has no such
     * escape hatch and is rejected at load time.
     *
